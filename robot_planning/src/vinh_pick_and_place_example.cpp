...
#include <robot_planning/pick_and_place_example.h>
...
{
  VinhPickAndPlaceExample::VinhPickAndPlaceExample(std::shared_ptr<tesseract_environment::Environment> env,
                                                   std::shared_ptr<tesseract_visualization::Visualization> plotter,
                                                   bool ifopt,
                                                   bool debug,
                                                   double box_size,
                                                   std::array<double, 2> box_position)
      : Example(std::move(env), std::move(plotter)), ifopt_(ifopt), debug_(debug), box_size_(box_size), box_position_(box_position)
  {
  }

  bool VinhPickAndPlaceExample::run()
  {
    /////////////
    /// SETUP ///
    /////////////
    ...
    // Get the exact 3D pose of the end effector (e.g., "tool0" or "link_6_t")
    if (current_state.link_transforms.find(LINK_END_EFFECTOR_NAME) == current_state.link_transforms.end())
    {
      CONSOLE_BRIDGE_logError("End effector link not found in transforms!");
      return false;
    }
    Eigen::Isometry3d start_pose = current_state.link_transforms.at(LINK_END_EFFECTOR_NAME);

    // 2. Create the "Up" Pose (Z + 0.02m)
    Eigen::Isometry3d up_pose = start_pose;
    // This adds 0.02m to the Global Z axis (lifting up relative to the world)
    up_pose.translation() += Eigen::Vector3d(0.0, 0.0, 0.02);

    // 3. Create Waypoints
    // Waypoint A: The current starting joint configuration
    StateWaypoint wp_start{joint_names, joint_pos};

    // Waypoint B: The target Cartesian pose (Up)
    CartesianWaypoint wp_up{up_pose};

    // 4. Create the Program
    CompositeInstruction program("DEFAULT", ManipulatorInfo("manipulator", LINK_BASE_NAME, LINK_END_EFFECTOR_NAME));

    // Instruction 1: Start at current state
    MoveInstruction instr_start(wp_start, MoveInstructionType::FREESPACE, "FREESPACE");
    instr_start.setDescription("Start at Initial Pose");

    // Instruction 2: Move Linear to Z+0.02
    MoveInstruction instr_move_up(wp_up, MoveInstructionType::LINEAR, "CARTESIAN");
    instr_move_up.setDescription("Move Up 0.02m");

    // Instruction 3: Move Linear back to Start
    // We use Cartesian movement to ensure a straight line back down
    CartesianWaypoint wp_return{start_pose};
    MoveInstruction instr_move_back(wp_return, MoveInstructionType::LINEAR, "CARTESIAN");
    instr_move_back.setDescription("Return to Start");

    // Add to program
    pick_program.push_back(instr_start);
    pick_program.push_back(instr_move_up);
    pick_program.push_back(instr_move_back);
